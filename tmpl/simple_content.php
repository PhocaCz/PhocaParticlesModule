...
if ($titleObject['title'] != '') {
    ...
    $titlePrefix    = (($items[0]->item_title_prefix ?? '') !== '') ? '<span class="phModParticlesTitlePrefix">' . htmlspecialchars($items[0]->item_title_prefix) . '</span>' : '';
    $titleSuffix    = (($items[0]->item_title_suffix ?? '') !== '') ? '<span class="phModParticlesTitleSuffix">' . htmlspecialchars($items[0]->item_title_suffix) . '</span>' : '';

    echo '<div class="phModParticlesTitle" '.$p['style_title'].'>'. $linkObjectItem['starttitle'] .$titlePrefix .PhocaParticlesHelper::completeValueContent($titleObjectItem['title'], $p['item_title_animation']). $titleSuffix . $linkObjectItem['end'] . '</div>';
}
