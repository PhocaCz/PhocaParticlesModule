...
// Main Title/Desc outside the grid
...
echo '<div class="phModParticlesFlipBoxItems">';
...
if (!empty($items)) {
    foreach ($items as $k => $v) {
        ...
        // The Flip Box Container (Perspective)
        echo '<div class="phModParticlesItem phModParticlesFlipBox ' . $boxItemClass . '" tabindex="0">';

            // The Inner Flipper (Preserve 3D)
            echo '<div class="phModParticlesFlipBoxInner">';

                // --- FRONT FACE ---
                echo '<div class="phModParticlesFlipBoxFront">';

                    if (($v->item_image ?? '') !== '') {
                        echo '<div class="phModParticlesFlipBoxImage"><img' . PhocaParticlesHelper::completeValueAttribute($p['item_image_animation']) . ' src="' . URI::base() . '/' . htmlspecialchars(strip_tags($v->item_image)) . '" alt="' . $titleObject['alt'] . '" /></div>';
                    ...
                    } else if (($v->item_icon_class ?? '') !== '') {
                        echo '<div class="phModParticlesFlipBoxIcon"><i class="' . htmlspecialchars(strip_tags($v->item_icon_class)) . '"></i></div>';
                    }
                ...
                // --- BACK FACE ---
                echo '<div class="phModParticlesFlipBoxBack">';

                    // Duplicate title on back if needed? varying designs. Let's keep it simple: Desc + Button
