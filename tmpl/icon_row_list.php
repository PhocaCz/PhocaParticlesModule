...
echo '<div class="phModParticlesIconRowList">';
...
if (!empty($items)) {
    foreach ($items as $k => $v) {
        ...
        echo '<div class="phModParticlesItem phModParticlesIconRowListItem' . $boxItemClass . '">';

        echo '<div class="phModParticlesIconRowListIcon">';
        if (($v->item_icon_class ?? '') !== '') {
            ...
            echo '<div class="phModParticlesImage" ' . $p['style_icon'] . '>' . $linkObject['starticon'] . '<img src="' . URI::base() . '/' . htmlspecialchars(strip_tags($v->item_image)) . '" alt="' . $titleObject['alt'] . '" />' . $linkObject['end'] . '</div>';
        }
        echo '</div>';

        echo '<div class="phModParticlesIconRowListText">';
